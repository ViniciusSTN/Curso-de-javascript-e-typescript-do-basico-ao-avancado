...
class UserController {
  ...
  async index(req, res) {
    try {
      ...
      const users = await _User2.default.findAll({ attributes: ['id', 'nome', 'email'] }); // busca somente esses tributos listados
      console.log(users);
  ...
  async show(req, res) {
    try {
      const user = await _User2.default.findByPk(req.params.id); // find by primary key
